  ...
  grouped_embeddings AS (
    ...
    FROM (
      SELECT
        ...
        re.text,
        re.similarity,
        SUM(CASE WHEN re.row_num = 1 THEN 0 ELSE EXTRACT(EPOCH FROM re."timestamp" - LAG(re."timestamp") OVER (PARTITION BY re.video_uuid ORDER BY re."timestamp")) END) 
          OVER (PARTITION BY re.video_uuid ORDER BY re."timestamp") / 120 AS grp
