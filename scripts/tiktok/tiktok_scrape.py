    ...
    try:
        ...
        response = client.beta.chat.completions.parse(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a content filter for political TikTok data.",
                },
                {"role": "user", "content": prompt},
            ],
            temperature=1,
        ...
        return response.choices[0].message.parsed.satisfies_criteria
...
async def process_user(api, keyword):
    ...
    async for user in api.search.users(keyword, count=1):
        try:
            ...
            if user_info["videos"]:
                user_info["trending_video"] = max(
                    user_info["videos"], key=lambda x: x["views"]
                )
...
async def search_users_and_content():
    ...
    try:
        ...
        results = {"trending_videos": [], "political_content": [], "user_data": []}
        ...
        hashtag_tasks = [
            process_hashtag(api, hashtag) for hashtag in political_hashtags
        ]
...
def process_comments_worker():
    while True:
        try:
            video_id, comments = video_queue.get(
                timeout=5
            )  # Wait for 5 seconds for new items
...
if __name__ == "__main__":
    ...
    with Pool(num_workers) as pool:
        workers = [
            pool.apply_async(process_comments_worker) for _ in range(num_workers)
        ]
